...
//            ruleList.add(createOperation);
        for (EwsRuleEntity ruleEntity : ewsRuleEntityList) {
            ...
            rule = this.transformRuleEntity(ruleEntity, ewsMail);
            createOperation = new CreateRuleOperation(rule);
            ...
            try {
                ...
                //执行规则更新
                EwsExContainer.getExchangeService(ewsMail.getEmail(), ewsMail.getPassword())
                        .updateInboxRules(ruleList, false);
//            EwsContainer.defaultExchangeService().update(ruleList,true);
            } catch (Exception e) {
                e.printStackTrace();
                return -1;
...
//        ewsEmailService.getMailConfigList()
        ...
        public Integer disabledRuleByEmAddr (EwsMailEntity ewsMail){
            ...
            for (Rule rule : ruleCollection) {
                ...
                System.out.println(rule.getId());
                System.out.println(rule.getIsEnabled());
                System.out.println(rule.getActions().getMoveToFolder().getUniqueId());
                rule.setIsEnabled(false);
//            rule.getActions().setMoveToFolder(new FolderId(WellKnownFolderName.Inbox));
                System.out.println("rule isEnabled must be false now:" + rule.getIsEnabled());
//            System.out.println("rule disabled:"+rule);
        ...
        public Rule transformRuleEntity (EwsRuleEntity ewsRuleEntity, EwsMailEntity ewsMail){
            ...
            System.out.println("impl priority:" + rule.getPriority());
...
//            EwsMailFolderRelation oneByConditionCode = ruleFolderRelationService.findOneByConditionCode(ewsRuleEntity.getRuleId(), moveToFolder, ewsMail.getMailId());
                List<EwsMailFolderRelation> folderRelation = mailFolderRelationService.findByMailId(ewsMail.getMailId());
                log.info("folderId in rule impl move:{}", folderRelation.get(0).getFolderId());
                ewsActionsEntity.setMoveToFolder(folderRelation.get(0).getFolderId());
            }
...
//            EwsMailFolderRelation oneByConditionCode = ruleFolderRelationService.findOneByConditionCode(ewsRuleEntity.getRuleId(), copyToFolder, ewsMail.getMailId());
                List<EwsMailFolderRelation> folderRelation = mailFolderRelationService.findByMailId(ewsMail.getMailId());
                log.info("folderId in rule impl copy:{}", folderRelation.get(0).getFolderId());
                ewsActionsEntity.setCopyToFolder(folderRelation.get(0).getFolderId());
            }
